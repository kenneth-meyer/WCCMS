...
//Merge "ellipsoid_30m.stl";
Merge "ellipsoid_dense10.stl";
CreateTopology;

//Mesh.CharacteristicLengthExtendFromBoundary = 0; //removing auto mesh sizing from stl file; was not fine enough imo.
//Mesh.CharacteristicLengthMax = 100; //not entirely sure how this works; need to figure out to get better mesh.
...
Physical Surface(1) = {1}; //I used to say that this was a box, think I was wrong.
//Dilate {{0, 0, 0}, 10} {Surface{1};}
...
//Mesh.CharacteristicLengthFactor = 10; //100 = coarse, .5 was original (is very dense)...is this working is the question

// this doesn't seem to be working rn

Characteristic Length{:} = 20;

Characteristic Length{PointsOf{Physical Volume{26};}}  = 20;
Characteristic Length{PointsOf{Physical Volume{28};}}  = 1;
Characteristic Length{PointsOf{Physical Surface{1};}} = 1; //don't think this changes anything
Characteristic Length{PointsOf{Physical Volume{thehole};}}  = 1;

//coloring attempts
//Color Red {Surface {s1,s2,s3,s4,s5,s6,s7,s8};} //nucleus: red
//Color Red { Volume{300}; } //nucleus, idk about the syntax though

//Color Grey50{ Surface{22}; }
//Color Purple{ Surface{ 201 }; }
//Color Red{ Surface{ s1,s2,s3,s4,s5,s6,s7,s8 }; }
//Color Yellow{ Volume{ 200 }; } // 200 or 28??
...
//attempting to alter the size of the mesh within the nucleus...looking odd in gmsh though.
//mixing volumes and surfaces, idk best way/how to do this.

/*
Field[1] = MathEval;
Field[1].F = "10";
Field[2] = Restrict;
//Field[2].IField = 1; //think this is supposed to be "InField", also it is 1 by default anyway
Field[2].VolumesList = {300};
Field[3] = MathEval;
Field[3].F = "100";
Field[4] = Restrict;
Field[4].SurfacesList = {28};
Background Field = 2;
*/

//Field[2].SurfacesList = {28}; //trying to change density of mesh in cytoplasm (??)